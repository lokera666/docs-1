...
<!-- You can also specify a custom agent image that this Buildkite hosted queue uses (a [private preview](/docs/agent/v3/buildkite-hosted/linux/custom-base-images#use-an-agent-image-specify-a-custom-image-for-a-queue) feature only), by passing its URL as the value to its `agentImageRef` parameter.
-->
